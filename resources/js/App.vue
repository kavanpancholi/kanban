    ...
    <div class="flex flex-grow px-10 mt-4 space-x-6 overflow-auto">
      <div class="flex flex-col flex-shrink-0 w-72" v-for="column in columns">
        <div class="flex items-center flex-shrink-0 h-10 px-2">
          ...
          <span
              class="flex items-center justify-center w-5 h-5 ml-2 text-sm font-semibold text-indigo-500 bg-white rounded bg-opacity-30">{{ column.cards.length }}</span>
          <button
        ...
        <div class="flex flex-col pb-2 overflow-auto">
          <div
              ...
              draggable="true" v-for="card in column.cards">
            <button
            ...
            <h4 class="mt-3 text-sm font-medium">{{ card.title }}</h4>
          </div>
        </div>
      </div>
